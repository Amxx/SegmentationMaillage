...
CGAL::Bbox_3 bbox(LCC& lcc)
{
	...
	typename LCC::Vertex_attribute_range::const_iterator it			= lcc.vertex_attributes().begin();
	typename LCC::Vertex_attribute_range::const_iterator itend	= lcc.vertex_attributes().end();
  if ( it != itend )
  {
...
class SimpleLCCViewerQt : public QGLViewer
{
  ...
  // Constructor/Destructor
		SimpleLCCViewerQt(LCC& alcc) :	QGLViewer(),
																		lcc(alcc),
																		wireframe(false),
																		flatShading(true),
																		edges(true),
	...
	protected :
		...
		void drawFacet(Dart_handle ADart, int AMark)
		{
			...
			for (	typename LCC::template Dart_of_orbit_range<1>::const_iterator it = lcc.template darts_of_orbit<1>(ADart).begin();
						it.cont();
						++it )
			{
		...
		void drawEdges(Dart_handle ADart)
		{
			...
			for (	typename LCC::template Dart_of_orbit_range<1>::iterator it = lcc.template darts_of_orbit<1>(ADart).begin();
						it.cont();
						++it)
			{
				Local_point p =  geomutils.get_point(it);
		...
		virtual void draw()
		{
			...
			typename LCC::Dart_range::iterator it_begin	= lcc.darts().begin();
			typename LCC::Dart_range::iterator it_end		= lcc.darts().end();
			for ( typename LCC::Dart_range::iterator it = it_begin; it != it_end; ++it )
			{
